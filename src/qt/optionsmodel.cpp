...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NDB);
    ...
    if (!settings.contains("nAnonymizeNodebaseAmount"))
        settings.setValue("nAnonymizeNodebaseAmount", 1000);

    nAnonymizeNodebaseAmount = settings.value("nAnonymizeNodebaseAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNodebaseAmount"))
        SoftSetArg("-anonymizenodebaseamount", settings.value("nAnonymizeNodebaseAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nodebase.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNodebaseAmount:
            return QVariant(nAnonymizeNodebaseAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNodebaseAmount:
            nAnonymizeNodebaseAmount = value.toInt();
            settings.setValue("nAnonymizeNodebaseAmount", nAnonymizeNodebaseAmount);
            emit anonymizeNodebaseAmountChanged(nAnonymizeNodebaseAmount);
            break;
        case CoinControlFeatures:
