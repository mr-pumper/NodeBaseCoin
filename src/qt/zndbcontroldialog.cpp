...
#include "ui_zndbcontroldialog.h"
...
std::list<std::string> ZNdbControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNdbControlDialog::listMints;

ZNdbControlDialog::ZNdbControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNdbControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNdbControlDialog::~ZNdbControlDialog()
{
    delete ui;
}

void ZNdbControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNdbControlDialog::updateList()
{
...
void ZNdbControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNdbControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNdb_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNdbControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNdbControlDialog::GetSelectedMints()
{
...
void ZNdbControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
