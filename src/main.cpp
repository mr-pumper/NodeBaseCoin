...
#if defined(NDEBUG)
#error "Nodebase cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in undb) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "Nodebase Signed Message:\n";
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zNdb with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zNdb spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zNDB spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 1 * COIN; //pow period
    } else if (nHeight > 1001 && nHeight <= 21600) {
        nSubsidy = 2 * COIN; // pow - 30 days
    } else if (nHeight > 21601 && nHeight <= 43200) {
        nSubsidy = 2.5 * COIN; // 30 - 60 days
    } else if (nHeight > 43201 && nHeight <= 64800) {
        nSubsidy = 3 * COIN; // 60 -90 days
    } else if (nHeight > 64801 && nHeight <= 100800) {
        nSubsidy = 5 * COIN; // 90 - 140 days
    } else if (nHeight > 100801 && nHeight <= 262800) {
        nSubsidy = 3 * COIN; // 140 days - 1 year
    } else if (nHeight > 262801 && nHeight <= 525600) {
        nSubsidy = 2.5 * COIN; // 1 - 2 years
    } else if (nHeight > 525601 && nHeight <= 788400) {
        nSubsidy = 2 * COIN; // 2 -3 years
    } else if (nHeight > 788401 && nHeight <= 1051200) {
        nSubsidy = 1.5 * COIN; // 3-4 years
    } else if (nHeight > 1051201 && nHeight <= 1314000) {
        nSubsidy = 1 * COIN;
    } else {
        nSubsidy = 0.5 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Nodebase
...
{
    RenameThread("nodebase-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZNDBMinted()
{
...
void RecalculateZNDBSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zNDB supply
...
bool RecalculateNDBSupply(int nHeightStart)
{
...
bool UpdateZNDBSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (Params().Zerocoin_Block_RecalculateAccumulators() >= 0 && pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateNDBSupply(Params().Zerocoin_StartHeight());
    }

    //Track zNDB money supply in the block index
    if (!UpdateZNDBSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zNDB supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zNdbSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert NDB to zNDB
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zNdb spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zNdb serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zNdb mints and %d zNdb spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Nodebase: We use certain sporks during IBD, so check to see if they are
