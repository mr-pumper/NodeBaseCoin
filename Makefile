...
pkgdatadir = $(datadir)/nodebase
pkgincludedir = $(includedir)/nodebase
pkglibdir = $(libdir)/nodebase
pkglibexecdir = $(libexecdir)/nodebase
...
CONFIG_HEADER = $(top_builddir)/src/config/nodebase-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/nodebase-config.h.in \
...
ACLOCAL = ${SHELL} /workspaces/eclipse/nodebase/nodebase-build/NodeBaseCoin/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /workspaces/eclipse/nodebase/nodebase-build/NodeBaseCoin/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /workspaces/eclipse/nodebase/nodebase-build/NodeBaseCoin/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /workspaces/eclipse/nodebase/nodebase-build/NodeBaseCoin/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = nodebase-cli
BITCOIN_DAEMON_NAME = nodebased
BITCOIN_GUI_NAME = nodebase-qt
BITCOIN_TX_NAME = nodebase-tx
...
MAINT = 
MAKEINFO = ${SHELL} /workspaces/eclipse/nodebase/nodebase-build/NodeBaseCoin/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = nodebase
PACKAGE_BUGREPORT = nodebase.tech
PACKAGE_NAME = Nodebase Core
PACKAGE_STRING = Nodebase Core 2.0.2
PACKAGE_TARNAME = nodebase
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /workspaces/eclipse/nodebase/nodebase-build/NodeBaseCoin
abs_srcdir = /workspaces/eclipse/nodebase/nodebase-build/NodeBaseCoin
abs_top_builddir = /workspaces/eclipse/nodebase/nodebase-build/NodeBaseCoin
abs_top_srcdir = /workspaces/eclipse/nodebase/nodebase-build/NodeBaseCoin
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /workspaces/eclipse/nodebase/nodebase-build/NodeBaseCoin/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_nodebase_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_nodebase_coverage.info test_nodebase.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Nodebase-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/nodebase-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/nodebase-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/nodebase-config.h
$(top_srcdir)/src/config/nodebase-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/nodebase-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Nodebase-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Nodebase-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Nodebase-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_nodebase.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_nodebase -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_nodebase_filtered.info: test_nodebase.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_nodebase_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_nodebase_coverage.info: baseline_filtered_combined.info test_nodebase_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_nodebase_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_nodebase_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_nodebase_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_nodebase.coverage/.dirstamp:  test_nodebase_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_nodebase.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_nodebase.coverage/ total.coverage/ $(OSX_APP)
